...
struct SegmentTree
{
    ...
    virtual int op(int a, int b) { cout << "what the fuck" << endl; return 120341235; };
};

struct SumTree : public SegmentTree
{
    SumTree(int arr[], int n)
    {
        build(1, 0, n - 1, arr);
        id = 0;
    }
...
int main() {
    ...
    SumTree sumTree(arr, n);
    cout << sumTree.op(3, 5) << endl;
